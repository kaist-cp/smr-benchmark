...
impl<K, V> Node<K, V> {
    ...
    fn protect_next<'g>(
        &self,
        index: usize,
        slot: &mut Shield<Self>,
        guard: &'g Guard,
    ) -> Result<Shared<'g, Self>, ShieldError> {
        let ptr = self.next()[index].load(Ordering::Acquire, guard);
        slot.defend(ptr, guard).map(|_| ptr)
    }
    ...
    V: Default + Copy,
{
    ...
    pub fn search_basic(&self, key: &K, cursor: &mut Cursor<K, V>, guard: &mut Guard) -> Option<V> {
        loop {
            match self.search_basic_inner(key, cursor, guard) {
    ...
    fn search_basic_inner(
        &self,
        key: &K,
        cursor: &mut Cursor<K, V>,
        guard: &Guard,
    ) -> Result<Option<V>, ShieldError> {
        let mut node = unsafe { self.entry.protect_next(0, &mut cursor.p, guard)?.deref() };
        while !node.is_leaf() {
            let next = node.protect_next(node.child_index(key), &mut cursor.l, guard)?;
            swap(&mut cursor.l, &mut cursor.p);
    ...
    fn search<'g>(
        ...
        loop {
            match self.search_inner(key, target, cursor, guard) {
                Ok(found) => return found,
                Err(_) => guard.repin(),
            }
        }
    ...
    fn search_inner<'g>(
        ...
        self.entry.protect_next(0, &mut cursor.l, guard)?;
        self.entry.protect_next(1, &mut cursor.s, guard)?;
        unsafe {
            cursor
                .p
                .defend_fake(Shared::from_usize(&self.entry as *const _ as usize))
        };
        cursor.gp.release();
        cursor.gp_p_idx = 0;
        cursor.p_l_idx = 0;
        ...
        while !unsafe { cursor.l.deref() }.is_leaf()
            && target
                .map(|target| target != cursor.l.shared())
                .unwrap_or(true)
        {
            swap(&mut cursor.gp, &mut cursor.p);
            swap(&mut cursor.p, &mut cursor.l);
            let l_node = unsafe { cursor.p.deref() };
            cursor.gp_p_idx = cursor.p_l_idx;
            cursor.p_l_idx = l_node.child_index(key);
            cursor.p_s_idx = Node::<K, V>::p_s_idx(cursor.p_l_idx);
            l_node.protect_next(cursor.p_l_idx, &mut cursor.l, guard)?;
            l_node.protect_next(cursor.p_s_idx, &mut cursor.s, guard)?;
        }

        if let Some(target) = target {
            Ok(cursor.l.shared() == target)
        } else {
    ...
    pub fn insert(
        ...
        loop {
            self.search(key, None, cursor, guard);
    ...
    fn insert_inner(
        ...
        if node.size.load(Ordering::Acquire) < Self::ABSORB_THRESHOLD {
            ...
            unsafe { guard.defer_destroy(cursor.l.shared()) };
            self.fix_tag_violation(
    ...
    fn fix_tag_violation(
        ...
        while let Some((search_key, viol)) = stack.pop() {
            ...
            if !found || cursor.l.shared().into_usize() != viol {
                // `viol` was replaced by another update.
                // We hand over responsibility for `viol` to that update.
                continue;
            }
            let (success, recur) = self.fix_tag_violation_inner(cursor, guard);
            if !success {
    ...
    fn fix_tag_violation_inner<'g>(
        ...
        debug_assert!(!cursor.gp.shared().is_null());
        let node = unsafe { cursor.l.deref() };
        let parent = unsafe { cursor.p.deref() };
        ...
        if !parent.weight {
            return (
                false,
                Some((parent.search_key, cursor.p.shared().into_usize())),
            );
        }
        ...
        if size <= Self::ABSORB_THRESHOLD {
            ...
            unsafe { guard.defer_destroy(cursor.l.shared()) };
            unsafe { guard.defer_destroy(cursor.p.shared()) };
            return (true, None);
        } else {
            // Split case.
    ...
    pub fn remove(&self, key: &K, cursor: &mut Cursor<K, V>, guard: &mut Guard) -> Option<V> {
        loop {
            self.search(key, None, cursor, guard);
            if cursor.val.is_none() {
                return None;
            }
            if let Ok(result) = self.remove_inner(key, cursor, guard) {
                cursor.val = result;
                return result;
            }
        }
    ...
    fn fix_underfull_violation(
        ...
        while let Some((search_key, viol)) = stack.pop() {
            ...
            if cursor.l.shared().into_usize() != viol {
                // `viol` was replaced by another update.
                // We hand over responsibility for `viol` to that update.
                continue;
            }
            let (success, recur) = self.fix_underfull_violation_inner(cursor, guard);
            if !success {
                stack.push((search_key, viol));
            }
            stack.extend(recur);
    ...
    fn fix_underfull_violation_inner(
        ...
        if parent.size.load(Ordering::Relaxed) < Self::UNDERFULL_THRESHOLD
            ...
            && cursor.p.shared() != self.entry.load_next(0, guard)
        {
            return (
                false,
                ArrayVec::from_iter(once((parent.search_key, cursor.p.shared().into_usize()))),
            );
        }
        ...
        if size < 2 * Self::UNDERFULL_THRESHOLD {
            ...
            if eq(gparent, &self.entry) && psize == 2 {
                ...
                unsafe {
                    guard.defer_destroy(cursor.l.shared());
                    guard.defer_destroy(cursor.p.shared());
                    guard.defer_destroy(cursor.s.shared());
                }

                drop((left_lock, right_lock, parent_lock, gparent_lock));
                return (
                    true,
                    ArrayVec::from_iter(once((node.search_key, new_node.into_usize()))),
                );
            } else {
                ...
                return (
                    ...
                    ArrayVec::from_iter(
                        [
                            (node.search_key, new_node.into_usize()),
                            (parent.search_key, new_parent.into_usize()),
                        ]
                        .into_iter(),
                    ),
                );
            ...
            let (new_left, new_right, pivot) = if left.is_leaf() {
                ...
                let new_left = {
                    let mut new_leaf = Owned::new(Node::leaf(true, left_size, Default::default()));
                    for i in 0..left_size {
                ...
                let (new_right, pivot) = {
                    ...
                    let mut new_leaf = Owned::new(Node::leaf(true, right_size, Default::default()));
                    for i in 0..right_size {
                        let (k, v) = kv_iter.next().unwrap();
                        new_leaf.init_key(i, Some(k));
                        new_leaf.init_value(i, v);
    ...
    V: Default + Copy,
{
    ...
    fn get<'g>(
        ...
        self.search_basic(key, handle, guard)
    }

    fn insert(&self, handle: &mut Self::Handle, key: K, value: V, guard: &mut Guard) -> bool {
        self.insert(&key, &value, handle, guard).is_none()
    }
