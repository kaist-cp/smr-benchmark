...
impl<K, V> Node<K, V> {
    ...
    fn load_next<'g>(&self, index: usize) -> Shared<Self> {
    ...
    V: Default + Copy,
{
    ...
    pub fn search_basic(&self, key: &K) -> Option<V> {
        let mut node = unsafe { self.entry.load_next(0).deref() };
        while let NodeKind::Internal { next } = &node.kind {
            let next = next[node.child_index(key)].load(Ordering::Acquire);
            node = unsafe { next.deref() };
        }
        node.read_consistent(key).1
    }

    fn search(&self, key: &K, target: Option<Shared<Node<K, V>>>) -> (bool, Cursor<K, V>) {
        let mut cursor = Cursor {
            l: self.entry.load_next(0),
            ...
            && target.map(|target| target != cursor.l).unwrap_or(true)
        {
            ...
            cursor.l = l_node.load_next(cursor.p_l_idx);
        }
    ...
    pub fn insert(&self, key: &K, value: &V) -> Option<V> {
        loop {
            let (_, cursor) = self.search(key, None);
            if let Some(value) = cursor.val {
                return Some(value);
            }
            match self.insert_inner(key, value, &cursor) {
    ...
    fn insert_inner(&self, key: &K, value: &V, cursor: &Cursor<K, V>) -> Result<Option<V>, ()> {
        ...
        if node.size.load(Ordering::Acquire) < Self::ABSORB_THRESHOLD {
            ...
            self.fix_tag_violation(new_internal);

            Ok(None)
        }
    }

    fn fix_tag_violation(&self, viol: Shared<Node<K, V>>) {
        loop {
            ...
            let (found, cursor) = self.search(&viol_node.search_key, Some(viol));
            if !found {
                return;
            }
            ...
            if !parent.weight {
                self.fix_tag_violation(cursor.p);
                continue;
            }
    ...
    pub fn remove(&self, key: &K) -> Option<V> {
        loop {
            ...
            match self.remove_inner(key, &cursor) {
                Ok(result) => return result,
                Err(()) => continue,
            }
        }
    ...
    fn remove_inner(&self, key: &K, cursor: &Cursor<K, V>) -> Result<Option<V>, ()> {
        let node = unsafe { cursor.l.deref() };
        let parent = unsafe { cursor.p.deref() };
        ...
        for i in 0..DEGREE {
            if node.get_key(i) == Some(*key) {
                ...
                if new_size == Self::UNDERFULL_THRESHOLD - 1 {
                    drop(node_lock);
                    self.fix_underfull_violation(cursor.l);
                }
                return Ok(Some(val));
            }
        }
    ...
    fn fix_underfull_violation(&self, viol: Shared<Node<K, V>>) {
        ...
        loop {
            ...
            // Search for `viol`.
            let (_, cursor) = self.search(&viol_node.search_key, Some(viol));
            let node = unsafe { cursor.l.deref() };
            let parent = unsafe { cursor.p.deref() };
            ...
            {
                self.fix_underfull_violation(cursor.p);
                continue;
            }
            ...
            let sibling_sh = parent.load_next(sibling_idx);
            ...
            if !parent.weight || !node.weight || !sibling.weight {
                ...
                self.fix_tag_violation(cursor.p);
                self.fix_tag_violation(cursor.l);
                self.fix_tag_violation(sibling_sh);
                continue;
            }
            ...
            if size < 2 * Self::UNDERFULL_THRESHOLD {
                ...
                if eq(gparent, &self.entry) && psize == 2 {
                    ...
                    drop((left_lock, right_lock, parent_lock, gparent_lock));
                    self.fix_underfull_violation(new_node);
                    return;
                } else {
                    ...
                    drop((left_lock, right_lock, parent_lock, gparent_lock));
                    self.fix_underfull_violation(new_node);
                    self.fix_underfull_violation(new_parent);
                    return;
                }
            } else {
                // Distribute
