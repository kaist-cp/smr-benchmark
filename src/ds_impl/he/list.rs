...
pub struct EraShields<'d, 'h> {
    prev_h: HazardEra<'d, 'h>,
    curr_h: HazardEra<'d, 'h>,
    next_h: HazardEra<'d, 'h>,
    // `anchor_h` and `anchor_next_h` are used for `find_harris`
    anchor_h: HazardEra<'d, 'h>,
    anchor_next_h: HazardEra<'d, 'h>,
}
...
impl<K, V> Cursor<K, V> {
    pub fn new<'d, 'h>(head: &Atomic<Node<K, V>>, shields: &mut EraShields<'d, 'h>) -> Self {
        Self {
    ...
    K: Ord + 'static,
{
    ...
    where
        F: Fn(&mut Cursor<K, V>, &K, &mut EraShields<'d, 'h>, &'h Handle<'d>) -> Result<bool, ()>,
    {
        loop {
