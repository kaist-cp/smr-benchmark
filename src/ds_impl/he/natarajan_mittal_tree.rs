...
use he::{Atomic, Handle, HazardEra, Shared};
    ...
    V: Clone,
{
    ...
    fn new_internal(left: Node<K, V>, right: Node<K, V>, handle: &Handle<'_>) -> Node<K, V> {
        Node {
            key: right.key.clone(),
            value: None,
...
pub struct EraShields<'d, 'h> {
    ancestor_h: HazardEra<'d, 'h>,
    successor_h: HazardEra<'d, 'h>,
    parent_h: HazardEra<'d, 'h>,
    leaf_h: HazardEra<'d, 'h>,
    curr_h: HazardEra<'d, 'h>,
}

impl<'d, 'h> EraShields<'d, 'h> {
...
where
    ...
    V: Clone + 'static,
{
    pub fn new<'d>(handle: &Handle<'d>) -> Self {
    ...
    fn cleanup<'d>(&self, record: &mut SeekRecord<K, V>, handle: &Handle<'d>) -> bool {
...
where
    ...
    V: Clone + 'static,
{
    type Shields<'d, 'h> = EraShields<'d, 'h>
    where
        'd: 'h;
...
mod tests {
    use super::NMTreeMap;
    use crate::ds_impl::he::concurrent_map;
