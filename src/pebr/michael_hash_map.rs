...
use super::harris_michael_list::{List, VRef};
        ...
        thread::scope(|s| {
            for t in 0..10 {
                s.spawn(move |_| {
                    ...
                    for i in keys {
                        assert!(hash_map.insert(i, i.to_string(), &mut crossbeam_pebr::pin()));
                    }
                });
            }
        })
            ...
            for t in 0..5 {
                s.spawn(move |_| {
                    ...
                    for i in keys {
                        assert_eq!(
                            i.to_string(),
                            hash_map.remove(&i, &mut crossbeam_pebr::pin()).unwrap()
                        );
                    }
                });
            }
            ...
            for t in 5..10 {
                s.spawn(move |_| {
                    ...
                    for i in keys {
                        assert_eq!(
                            i.to_string(),
                            *hash_map.get(&i, &mut crossbeam_pebr::pin()).unwrap()
                        );
                    }
                });
            }
