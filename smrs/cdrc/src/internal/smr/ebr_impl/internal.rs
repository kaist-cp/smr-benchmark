...
static MANUAL_EVENTS_BETWEEN_COLLECT: AtomicUsize = AtomicUsize::new(128);
...
pub fn set_manual_collection_interval(interval: usize) {
    MANUAL_EVENTS_BETWEEN_COLLECT.store(interval, Ordering::Relaxed);
}
...
impl Local {
    ...
    fn counts_between_try_advance() -> usize {
        MAX_OBJECTS.load(Ordering::Relaxed) * 2
    }
